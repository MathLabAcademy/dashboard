...
function BatchClassEnrollmentBulkAddModal({
  batchClassId,
  year,
  createBatchClassEnrollmentBulk,
  refreshData,
}) {
  ...
  const onSubmit = useCallback(
    async (values, actions) => {
      ...
      try {
        await createBatchClassEnrollmentBulk({
          batchClassId,
          year,
          ...values,
        })
    ...
    [batchClassId, createBatchClassEnrollmentBulk, handle, refreshData, year]
  )
    ...
    <Permit roles="teacher,analyst">
      ...
      >
        {({ status, isSubmitting, isValid }) => (
          ...
          >
            <Modal.Header>
              Add Bulk Batch Class Enrollment for {year}
            </Modal.Header>
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(BatchClassEnrollmentBulkAddModal)
